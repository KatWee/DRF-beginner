...
class TestModel(APITestCase):
    ...
    def test_raises_error_with_message_when_no_email_is_supplied(self):
        with self.assertRaisesMessage(ValueError,'The given email must be set'):
            User.objects.create_user(username="cryce", email='',password='password123!@')
    
    def test_creates_super_user_with_is_staff_status(self):
        with self.assertRaisesMessage(ValueError,'Superuser must have is_staff=True.'):
            User.objects.create_superuser(username="username", email='crycetruly@gmail.com',password='password123!@', is_staff=False)
        
    def test_creates_super_user_with_super_user_status(self):
        with self.assertRaisesMessage(ValueError,'Superuser must have is_superuser=True.'):
            User.objects.create_superuser(username="username", email='crycetruly@gmail.com',password='password123!@', is_superuser=False)
